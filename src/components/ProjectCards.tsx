      ...
      {projectsList.map((item, index) => (
        <Link to={`/projects/${item.route}`}>
          <motion.div
            initial={{
              opacity:
                isProjectsPage && window.innerWidth <= 768 ? 1 : initialOpacity,
            }}
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              ...
              <p
                className={`font-textFont text-sm text-lightText dark:text-darkText rounded-md py-1 px-3 mt-1 bg-[#e6e6e6] dark:bg-[#3333339A] w-fit max-sm:text-xs max-sm:px-2`}
              >
                {item.category}
              </p>
            </div>
